...
class DCEM(BaseModule):
    def __init__(self, in_channels=None, head_num=2):
        super(DCEM, self).__init__()
        self.head_num = head_num
        self.adaptive_scale = CEM()
        self.wave_refine = DEM(in_channels=in_channels)
...
class DCEFPN(BaseModule):
    ...
    def __init__(self,
        ...
        self.feature_refine = nn.ModuleList([DCEM(out_channels) for _ in range(3 + 1)])
        self.adaptiveContext = CEM()
        self.diffConv = nn.ModuleList(ConvModule(out_channels, out_channels, 3, 1, 1) for _ in range(3))
        self.contrast_factor = 2
        # 差分图归一化和激活
        self.diff_norm = nn.Sequential(
            nn.Sigmoid(),  # 将差分图归一化到[0,1]
        )
